...
const UploadVideo = () => {
  ...
  const isChapterInUse = (chapter) => {
    return usedChapterRanges.some(
      (range) => range.book === selectedBook && chapter == range.start
    );
  };
  ...
  return (
    <div
      style={{
        backgroundImage: `url(/images/ava3.jpg)`,
        backgroundOpacity: "50",
        objectFit: "fill",
        backgroundRepeat: "no-repeat",
        backgroundSize: "cover",
      }}
      className="m-auto w-full object-contain brightness-10  min-h-screen flex flex-col gap-2 text-center   md:px-28 items-center bg-gradient-to-b from-amber-300 to-[#c9822c] font-sniglet  md:pt-20"
    >
      {uploaded && (
        <div className="fixed z-[10000] inset-0 bg-darkbg bg-opacity-50 flex items-center justify-center px-5">
          <div className="bg-white py-8 px-5 md:px-16  rounded-lg shadow-lg flex flex-col items-center justify-center gap-1">
            <h3 className="text-4xl md:text-7xl text-center font-modak text-[#0081EE]">
              Gloraaay!
            </h3>
            <p className="text-center font-lucky font-light text-pinkbg text-lg">
            ...
            <button
              ...
              className="bg-gradient-to-tr from-[#0081EE] to-[#88CEDF]  px-5 py-2 rounded-xl text-black font-bold mt-5  border-2 shadow-lg shadow-slate-500 text-lg"
            >
              Upload another chapter
            </button>
            <Link
              href="/record"
              className=" mt-3 border-2 border-darkbg px-5 py-2 rounded-xl hover:bg-slate-200"
            >
              Record Instead
            </Link>
          </div>
      ...
      {fiesta ? (
        <div className="col-span-2 flex flex-col gap-3 md:w-[40rem] bg-[#f8f8f8] md:mt-10 h-screen md:h-auto p-10 text-left">
          <h2 className="font-lucky md:text-4xl text-3xl text-center text-darkbg">
            SELECT A CHAPTER TO UPLOAD
          </h2>
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              className="p-3 capitalize focus:outline-pinkbg focus:outline-1"
              disabled={user?.fullName}
              value={kidFullname || user?.fullName}
              onChange={(e) => setKidFullname(e.target.value)}
            />
          </div>

          <label htmlFor="fullname">
            Which Book in the Bible did you read?
          </label>
          <select
            value={selectedBook}
            className="p-3"
          ...
          <div className="flex items-center gap-4">
            <select
              className="p-3 w-full disabled:opacity-50 "
            ...
            >
              ...
              {chapters.map((chapter, index) => (
                <option
                  className="disabled:line-through disabled:cursor-not-allowed disabled:bg-slate-200"
                  key={index}
                  value={chapter}
                  disabled={isChapterInUse(chapter)}
                >
                  chapter {chapter}{" "}
                  {isChapterInUse(chapter) && "has been selected already"}
                </option>
              ))}
            </select>
          </div>
          <button
            onClick={() => setFiesta(false)}
            disabled={!selectedBook && !selectedChapterStart}
            className=" text-black rounded-md px-5 py-2.5 text-xl font-lucky  bg-gradient-to-tr from-[#EE7822] to-[#EFB741] active:bg-gradient-to-bl hover:rounded disabled:opacity-20"
          >
            Next
          </button>
        </div>
        ...
        <div className="bg-white w-full h-[90vh] md:h-full bg-opacity-60 rounded-3xl md:py-7">
          <h2 className="text-3xl bg-slate-500 md:bg-transparent w-full md:w-auto md:text-6xl py-5 md:py-0 font-lucky text-white md:text-slate-900">
            Upload your video
          </h2>
          {!videoSrc && (
            <h3 className="md:text-2xl my-1.5 md:font-lucky text-darkbg flex flex-col items-center gap-4 md:flex-row mx-auto w-fit">
              Or you can choose to{" "}
              <Link
                href="/record"
                className="font-bold text-white tracking-wider bg-pinkbg px-5 py-2 rounded-2xl"
              >
                Record your video
              </Link>
            </h3>
          ...
          {!videoSrc ? (
            <label
              htmlFor="fileInput"
              className={` mx-auto mt-5 md:mt-5 px-5 py-5 flex flex-col items-center justify-center w-[95%] md:w-[70%]  md:h-[450px] shadow-lg shadow-gray-800 bg-amber-200  text-center  border-2 border-dashed rounded-lg cursor-pointer ${
                dragging
                  ? "border-red-500 bg-amber-100"
            ...
            >
              <BiSolidCloudUpload
                size={50}
                color={dragging ? "#EE9F35" : "#333"}
              />
              <p className="mt-4 text-2xl text-slate-800 font-bold">
            ...
            <div className="md:mx-0  mx-auto  mt-7 md:mt-5 items-stretch gap-7 p-3 cursor-pointer rounded-2xl relative h-full  ">
              ...
              <button
                ...
                className="bg-gradient-to-bl from-[#0081EE] to-[#88CEDF] px-10 py-2 rounded-full text-slate-800m font-bold mt-5 border-2 shadow-lg shadow-slate-400 text-lg"
              >
                Upload Now
              </button>
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white py-6 px-10 rounded-lg shadow-lg">
            <h2 className="text-4xl font-bold mb-4 font-lucky text-pinkbg">
              Uploading Video...
            </h2>
