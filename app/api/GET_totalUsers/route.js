  ...
  try {
    ...
    if (influencerId) {
      if (!influencerIds.includes(influencerId)) {
        return new Response(
          JSON.stringify({ message: "Unauthorised pathway/influencer!" }),
          {
            status: 400,
      ...
      const influencerKingsChatUsers = await InfluencerKingsChat.find({
        influencer: influencerId,
      });

      // Combine the results
      const usersForInfluencer = [
        ...influencerEmailUsers,
        ...influencerKingsChatUsers,
      ];
    ...
    const totalInfluencerKingsChatUsers =
      await InfluencerKingsChat.countDocuments();
    ...
    for (const type of socialMediaTypes) {
      const emailCount = await InfluencerUser.countDocuments({
        influencerId: type,
      });
      const kcCount = await InfluencerKingsChat.countDocuments({
        influencer: type,
      });
      socialMediaCounts[`influencersThrough${type.toUpperCase()}`] =
        emailCount + kcCount;
    }

    // Calculate regular influencer counts (excluding social media types)
    const regularInfluencerIds = influencerIds.filter(
      (id) => !socialMediaTypes.includes(id)
    );
    ...
    for (const id of regularInfluencerIds) {
      totalRegularInfluencerUsers += await InfluencerUser.countDocuments({
        influencerId: id,
      });
      totalRegularInfluencerKingsChatUsers +=
        await InfluencerKingsChat.countDocuments({ influencer: id });
    }

    const totalUsersThroughRegularInfluencers =
      totalRegularInfluencerUsers + totalRegularInfluencerKingsChatUsers;
    const totalUsersThroughInfluencers =
      totalInfluencerUsers + totalInfluencerKingsChatUsers;
    const totalUsersThroughUsualRoutes = totalUsers + totalKingsChatUsers;
    const totalAllUsers =
      totalUsers +
      totalKingsChatUsers +
      totalInfluencerUsers +
      totalInfluencerKingsChatUsers;
